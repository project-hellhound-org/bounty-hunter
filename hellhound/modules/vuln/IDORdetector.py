...
CATEGORY    = "vuln"
VERSION     = "1.2"
DESCRIPTION = "Dual-session IDOR detector (path segments, query params, body keys)"
...
OPTIONS = [
    {"name": "cookie_a",      "type": str,   "default": None,  "required": True,
     "help": "Session token for User A (required — primary test session)"},
    ...
    {"name": "login_url_a",   "type": str,   "default": None,  "required": False,
     "help": "Explicit login URL (User A)"},
    ...
    {"name": "write_probe",   "type": bool,  "default": False, "required": False,
     "help": "Include POST/PUT write-probe tests"},
...
# ─────────────────────────────────────────────────────────────────────────────
# TERMINAL HELPERS
# Plain-text wrappers — no colorama dependency.
# All color/formatting is handled by the Hellhound console/emit layer.
# These exist so the module's internal verbose output works stand-alone too.
# ─────────────────────────────────────────────────────────────────────────────

class C:
    """ANSI color constants used by internal verbose output."""
    RESET   = "[0m"
    BOLD    = "[1m"
    DIM     = "[2m"
    RED     = "[31m"
...
def tprint(*args, **kwargs):
    """Thread-safe print for module internal verbose output."""
    import threading
    _tprint_lock = getattr(tprint, '_lock', None)
    if _tprint_lock is None:
        tprint._lock = threading.Lock()
        _tprint_lock = tprint._lock
    with _tprint_lock:
        print(*args, **kwargs)
...
def ok(msg):
    return color(f"[✓] {msg}", C.BGREEN, C.BOLD)

def warn(msg):
    return color(f"[!] {msg}", C.BYELLOW, C.BOLD)

def info(msg):
    return color(f"[•] {msg}", C.CYAN)

def err(msg):
    return color(f"[✖] {msg}", C.BRED, C.BOLD)

def found(msg):
    return color(f"[FOUND] {msg}", C.BRED, C.BOLD)
...
# Used across: Crawler, IDORSurfaceAnalyser, IDHarvestPass, IDORTester,
#              SpiderBridge, _spider_intel_to_endpoints
# ─────────────────────────────────────────────────────────────────────────────

# Pure numeric ID segment — "42", "1001"
...
# Slug — alphanumeric+hyphens/underscores with at least one digit
_SLUG_RE = re.compile(r'^[a-zA-Z0-9][a-zA-Z0-9_\-]{1,60}$')

# Numeric segment inside a URL path — /users/42/profile  → group(1)="42"
_PATH_NUMERIC_RE = re.compile(r'/(\d+)(?:/|$)')

# UUID segment inside a URL path → group(1)=uuid
_PATH_UUID_RE = re.compile(
    r'/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:/|$)',
    re.I
)

# Slug segment inside a URL path
_PATH_SLUG_RE = re.compile(r'/([a-zA-Z][a-zA-Z0-9_\-]{1,60})(?:/|$)')

# Parameter names that indicate IDOR-testable object references
_IDOR_PARAM_NAME_RE = re.compile(
    r'(?:^|_)(?:id|uid|user_?id|account_?id|profile_?id|order_?id|item_?id'
    r'|object_?id|resource_?id|doc_?id|file_?id|msg_?id|message_?id'
    r'|ticket_?id|invoice_?id|payment_?id|booking_?id|session_?id'
    r'|customer_?id|client_?id|owner_?id|ref|uuid|guid|token|key'
    r'|record|entry|entity|object|resource|doc|file|post|comment'
    r'|thread|forum|product|listing|ad|campaign|group|team|org'
    r'|project|task|issue|ticket|report|invoice|payment|subscription'
    r'|transaction|transfer|withdrawal|deposit|card|account|wallet)(?:$|_)',
    re.I
)

# Auth-related param names — skip during injection
_AUTH_RE = re.compile(
    r'(?:password|passwd|pass|token|csrf|secret|auth|captcha|otp|pin|code)',
    re.I
)

# Strip sanitization suffixes agents append to param names
...
# Spider v12 bucketed param ordering
        ...
        for target in idor_targets:
            ...
            for tampered_id in candidates:
                ...
                if is_idor:
                    ...
                    _poc_label, _poc_cmd = self._build_poc_curl(
                        t_url, method,
...
class SpiderBridge:
    ...
    def load(self, filepath, cli_target=None):
        ...
        try:
            ...
            tprint(f"  {err(f'--spider-json: file not found: {filepath}')}")
            raise RuntimeError("Fatal error")
        except json.JSONDecodeError as e:
            tprint(f"  {err(f'--spider-json: JSON parse error: {e}')}")
            raise RuntimeError("Fatal error")
        ...
        if isinstance(raw, dict):
            ...
            tprint(f"  {err('--spider-json: unrecognised JSON structure (expected object or array)')}")
            raise RuntimeError("Fatal error")
...
def _spider_intel_to_endpoints(intel, target):
    """
    Convert Spider intel dict → IDOR endpoint list using SpiderBridge logic.
    Handles bucketed params, auth_required flags, path-based ID hints.
    """
    bridge   = SpiderBridge()
    entries  = intel.get("endpoints", [])
    base_url = target.rstrip("/")

    # SpiderBridge.load() expects a filepath — build a thin in-memory version
    # by duplicating its parse logic directly with the dict we already have.
    endpoints     = []
    cli_parsed    = urllib.parse.urlparse(target)
    ...
    for entry in entries:
        ...
        # Build endpoint dict using SpiderBridge param parsing
        # (re-use bridge's _strip and _AUTH_RE for consistency)
        raw_params      = entry.get("params") or {}
        params          = {}
        priority_params = []
        id_hints_ep     = []
        ...
        if isinstance(raw_params, dict):
            ...
            if is_bucketed:
                for bucket in _BUCKET_ORDER:
                    for pname in (raw_params.get(bucket) or []):
                        pk = bridge._strip(pname)
                        if not pk or (bridge._AUTH_RE.search(pk) and bucket == "form"):
                            continue
                        if pk not in params:
                            params[pk] = "test"
                ...
                for pk, pv in raw_params.items():
                    pk = bridge._strip(pk)
                    if not pk or bridge._AUTH_RE.search(pk):
                        continue
        ...
        qs = urllib.parse.parse_qs(
            urllib.parse.urlparse(ep_url).query, keep_blank_values=True)
        for pk, pvlist in qs.items():
            pk = bridge._strip(pk)
            if pk and pk not in params:
                params[pk] = pvlist[0] if pvlist else "test"
                if _IDOR_PARAM_NAME_RE.match(pk) and pk not in priority_params:
                    priority_params.append(pk)
        clean_url = urllib.parse.urlunparse(
            urllib.parse.urlparse(ep_url)._replace(query="", fragment=""))

        # Synthesize IDOR hint for auth-required endpoints with no params
        is_auth_ep  = bool(entry.get("auth_required")) or bool(entry.get("parameter_sensitive"))
        has_path_id = bool(
...
def run(target, emit, options=None, stop_check=None, pause_check=None):
    ...
    options keys (all optional — see OPTIONS):
      cookie_a/b          : pre-captured session tokens
      login_user/pass_a/b : credentials for auto-login
      login_url_a         : explicit login URL
      auto_register       : create two test accounts
      timeout             : HTTP timeout (default 10)
      threads             : concurrent threads (default 8)
      delay               : inter-request delay
      write_probe         : test POST/PUT endpoints
    ...
    emit.section("IDOR Detector")

    ua          = None
    bare_client = HTTPClient(timeout=opts.get("timeout", 10), user_agent=ua)
    ...
    if cookie_a:
        client_a = HTTPClient(timeout=opts.get("timeout", 10),
                              cookie=cookie_a, extra_header=header_a, user_agent=ua)
        emit.info("User A: token loaded directly")
        if cookie_b:
            client_b = HTTPClient(timeout=opts.get("timeout", 10),
                                  cookie=cookie_b, extra_header=header_b, user_agent=ua)
            emit.info("User B: token loaded directly — dual-session IDOR enabled")
        else:
        ...
        if ok_a:
            ...
            emit.warn("User A login failed — running unauthenticated")
            client_a = bare_client.clone_no_auth()
    ...
    if not endpoints:
        ...
        from_crawler = Crawler(client_a, target, depth=3,
                               threads=opts.get("threads", 8), max_pages=200)
        endpoints = from_crawler.crawl()
        if not endpoints:
            emit.warn("No endpoints discovered")
    ...
    emit.info(f"High-signal: {high} | Medium: {medium} | Low: {low} endpoints")

    if not targets_list:
        emit.warn("No IDOR surface detected")
    ...
    # ── Build return intel ─────────────────────────────────────────────
    risk = sum(10 if f.get("severity") in ("Critical", "High") else
    ...
    for f in findings:
        emit.always_success(
            f"CONFIRMED: {f.get('vuln_type','IDOR')} @ {f.get('url','')} "
            f"[{f.get('severity','High')}]")

    return {
        "raw": f"{len(findings)} IDOR finding(s)",
        "intel": {
