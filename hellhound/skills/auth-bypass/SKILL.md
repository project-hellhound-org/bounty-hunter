...
3. **Complete the Authentication Chain:**
   - If a reset token/code is leaked in the response, submit it to the application's discovered reset endpoint:
     ...
     ```
   - **Mandatory Login Verification:** Log in via the discovered login endpoint (e.g. `<subpath>/api/auth/login`, ensuring the active application subpath is included in the URL) using the updated credentials:
     ```json
     {
       "tool": "curl",
       "args": {
         "url": "<discovered_login_endpoint_e.g._/pulse/api/auth/login>",
         "method": "POST",
         "headers": {"Content-Type": "application/json"},
         "json": {
...
## 13. INSECURE RANDOMNESS — TOKEN PREDICTION MECHANICS
...
## 14. FIELD MANIPULATION & CREDENTIAL TESTING CHEAT SHEET
...
### Password Field Truncation (bcrypt 72-Byte Limit)
```
bcrypt ignores input beyond 72 bytes.
- Register with password "A"*100
- Login with password "A"*72 -> matches hash!
```
